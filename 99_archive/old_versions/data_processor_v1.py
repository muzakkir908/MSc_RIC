...
class ImprovedGameDataProcessor:
    ...
    def check_data_quality(self):
        ...
        # Check GPU data quality
        gpu_zero_ratio = (self.df['gpu_percent'] == 0).mean()
        self.data_quality['gpu_working'] = gpu_zero_ratio < 0.8
        if not self.data_quality['gpu_working']:
            print(f"⚠️ GPU monitoring issue: {gpu_zero_ratio*100:.1f}% zero values")
        else:
            print("✅ GPU data quality good")
    ...
    def clean_and_fix_data(self):
        ...
        # Handle GPU data issues
        if not self.data_quality['gpu_working']:
            # Estimate GPU usage based on activity and system load
            self.df['gpu_percent_estimated'] = np.where(
                self.df['gpu_percent'] == 0,
                np.clip(
                    self.df['activity_score'] * 60 + 
                    self.df['cpu_percent'] * 0.3 +
                    np.random.normal(0, 5, len(self.df)), 
                    0, 100
                ),
                self.df['gpu_percent']
            )
            print("✅ Estimated GPU usage for missing data")
        else:
            self.df['gpu_percent_estimated'] = self.df['gpu_percent']
    ...
    def create_advanced_features(self):
        ...
        self.df['system_stress'] = np.clip(
            ...
            (self.df['gpu_percent_estimated'] / 100) * 0.4,
            0, 1
        )
        ...
        self.df['performance_risk'] = (
            (self.df['ping_ms'] > 80).astype(float) * 0.4 +
            (self.df['cpu_percent'] > 75).astype(float) * 0.3 +
            (self.df['gpu_percent_estimated'] > 85).astype(float) * 0.3
        )
    ...
    def create_rl_states_and_actions(self):
        ...
        self.df['optimal_action'] = np.where(
            self.df['performance_risk'] > 0.7,
            2,  # High slice for high risk
            np.where(
                (self.df['is_combat']) | (self.df['performance_risk'] > 0.3),
    ...
    def visualize_comprehensive_analysis(self):
        ...
        action_labels = ['Low Slice', 'Medium Slice', 'High Slice']
        ...
        system_metrics = ['cpu_percent', 'memory_percent', 'gpu_percent_estimated']
        system_data = [self.df[metric] for metric in system_metrics]
        labels = ['CPU', 'Memory', 'GPU (Est.)']
        ...
        ax8.plot(sample_indices, self.df['system_stress'].iloc[sample_indices], label='System Stress', alpha=0.8)
        ...
        plt.tight_layout()
        plt.savefig('comprehensive_gaming_analysis.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def print_final_report(self):
        ...
        print(f"   GPU Usage: {self.df['gpu_percent_estimated'].mean():.1f}%")
        ...
        action_names = ['Low Slice (Cheap)', 'Medium Slice (Balanced)', 'High Slice (Premium)']
        for i, (action, count) in enumerate(action_dist.items()):
            print(f"   {action_names[i]}: {count:,} times ({count/total_actions*100:.1f}%)")
        ...
        if not self.data_quality['gpu_working']:
            print(f"   🎮 GPU monitoring needs improvement for better system load prediction")
...
def main():
    ...
    csv_file = input("Enter the path to your merged CSV file: ").strip()
    ...
    try:
        ...
        print(f"   • comprehensive_gaming_analysis.png - Visualizations")
